...
from .entities import Fiche, Bonification, Bonus

repertoire_fiche = Path(__file__).parent / "../etc/db/repertoire_fiche.csv"
repertoire_bonification = Path(__file__).parent / "../etc/db/repertoire_bonification.csv"
repertoire_bonus = Path(__file__).parent / "../etc/db/repertoire_bonus.csv"
...
class BonificationRepository:
    ...
    def search(self, params: dict = {}) -> list[Bonification]:
        def fn(row) -> bool:
            if params.get("code", None) and params["code"] != row["code"]:
                return False
            if params.get("fiche", None) and params["fiche"] != row["code_fiche"]:
                return False
            if params.get("date", None):
                date = datetime.fromisoformat(params["date"])
                start = row["date_debut"]
...
class BonusRepository:
    def __init__(self):
        self.db = csv_reader(repertoire_bonus).to_dict("records")

    def __new__(cls):
        if not hasattr(cls, "instance"):
            cls.instance = super(BonusRepository, cls).__new__(cls)
        return cls.instance

    def find(self, id: str) -> Bonus | None:
        collection = list(filter(lambda x: x["id"] == id, self.db))
        return self.to_entity(collection[0]) if len(collection) > 0 else None

    def search(self, params: dict = {}) -> list[Bonus]:
        def fn(row) -> bool:
            if params.get("code", None) and params["code"] != row["code"]:
                return False
            if params.get("date", None):
    ...
    def to_entity(self, data: dict) -> Bonus:
        return Bonus(
            id=data["id"],
            code=data["code"],
            nom=data["nom"],
